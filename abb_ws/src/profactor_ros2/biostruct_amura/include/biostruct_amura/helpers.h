...
std::vector<double> rad2deg(const std::vector<double>& degrees) {
  constexpr double RAD2DEG = 180.0 / M_PI;
  std::vector<double> radians(degrees.size());
  std::transform(degrees.begin(), degrees.end(), radians.begin(),
                 [=](double d) { return d * RAD2DEG; });
  return radians;
}
...
std::unordered_map<std::string, std::vector<double>> joint_values_deg = {
  ...
  {"refPickingPose", {-42.0, 31.0, 7.0, 0.0, 52.0, -42.0}}, // to seed for faster IK on PickingPose
  {"PickingPose", {-42.0, 31.0, 7.0, 0.0, 52.0, -42.0}}, // just for simulation purposes
  {"refPlacingPose", {49.0, 49.0, 2.0, 241.0, -25.0, -185.0}},
  // {"bottomPlacingPose", {49.0, 49.0, 2.0, 241.0, -25.0, -185.0}}, 
  // {"sternPlacingPose", {45.0, 63.0, -16.0, 232.0, -105.0, -192.0}},
  // {"sidewallPlacingPose", {50.0, 50.0, 2.0, 156.0, -71.0, -113.0}},
  {"visionPose", {-40.0, 38.0, -54.0, 0.0, 106.0, -40.0}} // at 1.25155 m
};
...
// apply collision objects to the scene (Amura Scene)
void addCollisionEnvironment(const std::string& base_link)
{
  PlanningSceneInterface psi;
  shape_msgs::msg::Mesh mesh_msg;

  // ---------- Helper Lambda ----------
  auto loadMesh = [&](const std::string& uri, double scale) {
    auto* mesh = shapes::createMeshFromResource(uri, Eigen::Vector3d(scale, scale, scale));
    if (!mesh) throw std::runtime_error("Failed to load STL at " + uri);
    shapes::ShapeMsg sm;
    shapes::constructMsgFromShape(mesh, sm);
    auto out = boost::get<shape_msgs::msg::Mesh>(sm);
    delete mesh;
    return out;
  };
  ...
  // ---------- MOULD ----------
  {
    const std::string mould_uri = "file://" + ws_path +
        "/src/profactor_ros2/biostruct_amura/meshes/CollisionMould.stl";
    ...
    obj.meshes.push_back(loadMesh(mould_uri, 1e-4));
    obj.mesh_poses.push_back(P);
    obj.operation = obj.ADD;

    if (psi.applyCollisionObject(obj))
      RCLCPP_INFO(rclcpp::get_logger("Collision Environment"), "Collision Mould added correctly!");
  }

  // ---------- TABLE ----------
  {
    /* loading table using the STL
    const std::string table_uri = "file://" + ws_path +
        "/src/profactor_ros2/biostruct_amura/meshes/CollisionTable.stl";
    ...
    obj.meshes.push_back(loadMesh(table_uri, 1e-3));
    obj.mesh_poses.push_back(P);
    obj.operation = obj.ADD;

    if (psi.applyCollisionObject(obj))
      RCLCPP_INFO(rclcpp::get_logger("Collision"), "Collision Table added correctly!");*/
    moveit_msgs::msg::CollisionObject obj;
    obj.header.frame_id = base_link;
    obj.id = "table";
    ...
    primitive.dimensions[primitive.BOX_X] = 3.250*2;
    primitive.dimensions[primitive.BOX_Y] = 0.650*2;
    primitive.dimensions[primitive.BOX_Z] = 0.478;

    // Compute pose (center of the box)
    geometry_msgs::msg::Pose P;
    P.orientation.w = 1.0;
    P.position.x = 1.011 + 3.250;
    P.position.y = - 2.044 + 0.650;
    P.position.z = 0.478/2;
    ...
    if (psi.applyCollisionObject(obj))
      RCLCPP_INFO(rclcpp::get_logger("Collision Environment"), "Collision Table (box) added correctly!");
  }
}
...
void publishAmuraDrapingScene(const rclcpp::Node::SharedPtr& node,
                              const std::string& base_link,
                              const moveit::core::RobotModelConstPtr& robot_model)
{
  ...
  const std::string cell_uri  = "file://" + ws_path +
      "/src/profactor_ros2/biostruct_amura/meshes/Drapecell.stl";
  const std::string human_uri = "file://" + ws_path +
      "/src/profactor_ros2/biostruct_amura/meshes/Human.stl";
  const std::string mould_uri = "file://" + ws_path +
      "/src/profactor_ros2/biostruct_amura/meshes/Mould.stl";
  const std::string table_uri = "file://" + ws_path +
      "/src/profactor_ros2/biostruct_amura/meshes/Table.stl";
  ...
  p.position.z = -0.422;
  T.translation() = Eigen::Vector3d(p.position.x, p.position.y, p.position.z);
  scene_tools.publishMesh(T, human_uri, rviz_visual_tools::GREEN, 0.001);
  scene_tools.trigger();
  ...
  p.position.z = 0.398;
  T.translation() = Eigen::Vector3d(p.position.x, p.position.y, p.position.z);
  scene_tools.publishMesh(T, mould_uri, rviz_visual_tools::CYAN, 0.001);
  scene_tools.trigger();
  ...
  p.position.z = 0.478;
  T.translation() = Eigen::Vector3d(p.position.x, p.position.y, p.position.z);
  scene_tools.publishMesh(T, table_uri, rviz_visual_tools::MAGENTA, 0.001);
  scene_tools.trigger();
...
// [UNUSED] alternative to wait_until_at_goal
