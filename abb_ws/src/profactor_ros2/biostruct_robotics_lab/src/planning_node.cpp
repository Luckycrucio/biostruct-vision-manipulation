...
int main(int argc, char * argv[])
{
    ...
    rclcpp::init(argc, argv);
    {
        ...
        const std::string title = "PROFACTOR_CELL";
        draw_title(title);
        moveit_visual_tools.trigger();

        // SHOW the Visuals for the scene
        publishRoboticsLabScene(node,
                                    LINK_BASE_NAME,
                                    robot_model);

        // ADD Collision Objects
        moveit::planning_interface::PlanningSceneInterface psi;
        addCollisionEnvironment(LINK_BASE_NAME, psi);
        ...
        std::string pkg_share =
            ament_index_cpp::get_package_share_directory("biostruct_robotics_lab");
                ...
                for (int i = 0; i < N_SAMPLES; ++i)
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            // tool_pose.header = tf.header;           // frame_id = LINK_BASE_NAME
                            // tool_pose.pose.position.x = tf.transform.translation.x;
                            // tool_pose.pose.position.y = tf.transform.translation.y;
                            // tool_pose.pose.position.z = tf.transform.translation.z;
                            // tool_pose.pose.orientation = tf.transform.rotation;

                            // tf_buffer->transform(
                            //     tool_pose,
                            //     tool_pose,              // now expressed in base_link
                            //     LINK_BASE_NAME,            // "base_link"
                            //     tf2::durationFromSec(0.05)
                            // );
                            // draw_poses({
                            //     {"link_6",   tool_pose.pose}
                            // });
            ...
            if (plan_msg.has_value()){
                ...
                if (picking_pose_msg.has_value())
                {
                    ...
                    try
                    {
                        ...
                        RCLCPP_INFO(
                            mqtt_logger,
                            "Decoded picking_pose wrt TCP: x=%.3f y=%.3f z=%.3f  rx=%.3f ry=%.3f rz=%.3f dx=%.3f dy=%.3f theta=%.3f",
                            pose.x, pose.y, pose.z, pose.rx, pose.ry, pose.rz, pose.dx, pose.dy, pose.theta
                        );
                    }
                    catch (const std::exception& e)
                    {
                    ...
                    // ---- Force vertical (remove roll & pitch, keep yaw) ----
                    tf2::Quaternion q(
                        pickingPose.pose.orientation.x,
                        pickingPose.pose.orientation.y,
                        pickingPose.pose.orientation.z,
                        pickingPose.pose.orientation.w);

                    double roll, pitch, yaw; 
                    tf2::Matrix3x3(q).getRPY(roll, pitch, yaw);
                    q.setRPY(0.0, 0.0, yaw);       // vertical
                    q.normalize();

                    // ---- Translate in base_link frame ----
                    pickingPose.pose.position.x += dy; // inversion from image to base_LINK
                    pickingPose.pose.position.y += dx; // inversion from image to base_LINK

                    // ---- Rotate about world Z axis ----
                    tf2::Quaternion q_delta;
                    q_delta.setRPY(0.0, 0.0, z_rot); // also tool rotate
                    q_delta.normalize();

                    q = q_delta * q;   // world-frame rotation
                    q.normalize();

                    // ---- Write back ----
                    pickingPose.pose.orientation.x = q.x();
                    pickingPose.pose.orientation.y = q.y();
                    pickingPose.pose.orientation.z = q.z();
                    pickingPose.pose.orientation.w = q.w();

                    // ---- Rotate about local Y axis by +90 deg (flip) ----
                    tf2::Quaternion q_yflip;
                    q_yflip.setRPY(0.0, M_PI_2, 0.0);
                    q_yflip.normalize();

                    q = q * q_yflip;   // local/tool-frame flip
                    q.normalize();
                    ...
                    pickingPose.pose.orientation.z = q.z();
                    pickingPose.pose.orientation.w = q.w();
                    pickingPose.pose.position.z += 0.5; // not colliding pose 


                    // draw pose for debug
                    semi_perm_draw_poses({
                        {"PickingPose", pickingPose.pose},
                        {"CameraPose", camera_pose.pose}
                        //{"PlyPose", plyPose.pose}
                    });
                    semi_perm_scene_tools.trigger();

                    // update the memory with the estimated picking pose [UNCOMMENT FOR REAL HARDWARE]

                    setJointValuesFromPose("RefPickingPose", pickingPose.pose,  "PickingPose",robot_model, PLANNING_GROUP, LINK_TOOL_NAME);
